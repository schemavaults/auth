...
export default function logout() {
  ...
  cy.is_authenticated().should("be.true", "User should be authenticated before logout");
  ...
  cy.wait(2000).then(() => {
    ...
    cy.wait("@logoutRequest", { timeout: 15000 }).then((interception) => {
      cy.wrap(interception.response?.statusCode).should("eq", 200, "Logout API request should return 200");
      cy.log(
      ...
      cy.is_authenticated().should("equal", false, "User should not be authenticated after logout");
    });
    return;
  });
}
