...
/**
 * Test-only endpoint that creates an email verification token for a user and returns
 * the raw token directly (bypassing email sending). This allows E2E tests to
 * exercise the email verification flow without needing to mock or intercept emails.
 *
...
export async function GET(
  _req: NextRequest,
  ctx: RouteContext<"/api/test/email-verification-token/[email]">,
  ...
  try {
    ...
    return NextResponse.json(
      { error: true, success: false, message: "Failed to create email verification token" },
      { status: 500 },
    );
  }
}
