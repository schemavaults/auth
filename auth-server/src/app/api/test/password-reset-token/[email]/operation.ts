...
/**
 * Test-only endpoint that creates a password reset token for a user and
 * returns the raw token directly (bypassing email sending). This allows
 * E2E tests to exercise the password reset flow without needing to mock or
 * intercept emails.
 */
export const getTestPasswordResetToken = defineOperation({
  method: "get",
  path: "/api/test/password-reset-token/{email}",
  summary: "Mint a password reset token",
  description:
    "Creates a password reset token for the user with the given e-mail address and returns it raw instead of e-mailing it, so the E2E suite can drive the reset flow without intercepting mail.",
  tags: [API_TAGS.testEnvironment],
  auth: publicAccess(TEST_ENVIRONMENT_ONLY_NOTE),
  request: { params: testUserEmailParams },
  responses: {
    200: { description: "The raw reset token", schema: TestTokenResponse },
