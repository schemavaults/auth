      ...
      if (!user.admin) {
        ...
        if (!userOwnsApp) {
          return NextResponse.json(
            {
              success: false,
              message: "You must own the app (organization owner or owning user) to connect it!",
            } satisfies ResourceCreationResponse,
            { status: 403 },
          );
        }
        ...
        if (!isPublicHardcoded) {
          ...
          if (!userOwnsApi) {
            return NextResponse.json(
              {
                success: false,
                message: "You must own both the app and the API server (organization owner or owning user) to connect them!",
              } satisfies ResourceCreationResponse,
              { status: 403 },
            );
          }
      ...
      if (!user.admin) {
        ...
        if (!userOwnsApp) {
          return NextResponse.json(
            {
              success: false,
              message: "You must own the app (organization owner or owning user) to disconnect it!",
            },
            { status: 403 },
          );
        }
        ...
        if (!isPublicHardcoded) {
          ...
          if (!userOwnsApi) {
            return NextResponse.json(
              {
                success: false,
                message: "You must own both the app and the API server (organization owner or owning user) to disconnect them!",
              },
              { status: 403 },
            );
          }
