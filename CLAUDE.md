...
The first-time *authorization consent screen* (`AppAuthorizationConsentScreen` in `auth-server/src/components/AppAuthorizationConsentScreen/`, rendered by `AuthForm` after login/register and by `AlreadyAuthenticatedOnLoginOrRegisterPage` for signed-in users) treats `app_name`/`app_description` as untrusted registrant-supplied text (labelled as unverified in the UI) and always shows the host of the flow's allowlisted `redirect_uri` (`formatRedirectDestination` in `auth-server/src/lib/oauth2/format-redirect-destination.ts`, exposed as `data-testid="consent-redirect-host"`) so users can see which site is really asking; both callers must pass `redirect_uri`.
