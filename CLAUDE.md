...
Organization owners add members through the invitation flow (`InviteMemberDialog` in `@schemavaults/auth-ui`, by email or user ID, via `POST /api/organizations/[organization_id]/invitations`; the invitee must accept). Global administrators additionally get a direct-add surface on `/org/[organization_id]`: the `AddExistingMemberCard` (`@schemavaults/auth-ui`, rendered only when the server-resolved `isAdmin` prop is true) lists every registered user from `GET /api/admin/users/list` in a searchable combobox (current members filtered out), takes an initial role (`member`/`owner`; the virtual `admin` role is never assignable), and submits to `POST /api/organizations/[organization_id]/members` (`{ uid, role }`, admin-only — org owners receive 403 and must use invitations). The handler creates the membership immediately (no acceptance), enforces the membership cap, rejects system organizations and existing members, and revokes any pending invitation for that user. Shared form schema/types: `addExistingMemberFormSchema`/`AddExistingMemberSubmitData` in `@schemavaults/auth-common`; the list fetchers are exposed as `useAllUsers()` and `useOrganizationMembers()` hooks from `@schemavaults/auth-ui`.
