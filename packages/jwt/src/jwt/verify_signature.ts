import { type JWTPayload, jwtVerify, type JWTVerifyResult, type CryptoKey, decodeProtectedHeader, ProtectedHeaderParameters } from "jose";
...
interface BaseVerifyJWTSignatureInputOptions<
...
export type VerifyJWTSignatureInputOptions<
  TokenType extends AuthTokenTypes,
> = VerifyJWTSignatureInputWithAllKeysOptions<TokenType> | VerifyJWTSignatureInputWithVerificationKeyOptions<TokenType>;
...
>({
  ...
  try {
    if ("jwt_keys" in opts && opts.jwt_keys instanceof JWT_Keys) {
      const verifierKeyPromise: Promise<CryptoKey> = opts.jwt_keys.verification_key;
  ...
  try {
    const verify_result: JWTVerifyResult<JWTPayload> = await jwtVerify(jwt, verification_key, {
      audience: opts.aud,
      issuer,
      subject: opts.sub,
