...
export function evaluateAuthMiddlewareRules(
  ...
  if (environment === "development") {
    console.log(
      "[AuthMiddleware] Evaluating current path against redirect rules...",
    );
  }
  ...
  const defaultPageSecurityLevel =
    "authed" as const satisfies PageSecurityLevel;
  ...
  try {
    ...
    if (
      rules.public.some((route: NavigationPath): boolean => {
        const isMatch: boolean = comparePath(currentPath, route);
        if (environment === "test" && isMatch) {
          console.log(
            `[AuthMiddleware] Current path "${currentPath.join("/")}" matches public route: `,
            route,
          );
        }
        return isMatch;
      ...
      rules.unauthed.some((route: NavigationPath): boolean => {
        const isMatch: boolean = comparePath(currentPath, route);
        if (environment === "test" && isMatch) {
          console.log(
            `[AuthMiddleware] Current path "${currentPath.join("/")}" matches unauthed route: `,
            route,
          );
        }
        return isMatch;
      ...
      rules.authed?.some((route: NavigationPath): boolean => {
        ...
        if (environment === "test" && isMatch) {
          console.log(
            `[AuthMiddleware] Current path "${currentPath.join("/")}" matches authed route: `,
            route,
          );
        }
        return isMatch;
      ...
      rules.api.some((route: NavigationPath): boolean => {
        const isMatch: boolean = comparePath(currentPath, route);
        if (environment === "test" && isMatch) {
          console.log(
            `[AuthMiddleware] Current path "${currentPath.join("/")}" matches api route: `,
            route,
          );
        }
        return isMatch;
      ...
      if (environment === "development") {
        console.log(
          `[AuthMiddleware] Current path "${currentPath.join("/")}" does not match any route rule-- treating as 'authed' by default`,
        );
      }
      pageSecurityLevel = "authed";
    }
