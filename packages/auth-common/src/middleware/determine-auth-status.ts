...
export interface DetermineAuthStatusInputOptions {
  ...
  // version, e.g. dev, prod, staging, test
...
export async function determineAuthStatus(
  ...
  if (opts.client_type === 'client') { // this validation is really loose!! if they think they're logged in that's good enough-- server will actually check and tell them
    if (
  ...
  } else if (opts.client_type === 'server') { // Actually validate the token
    ...
    const token_sources: readonly PotentiallyValidTokenSource[] = opts.token_sources;
    ...
    throw new Error("Invalid environment to run auth middleware auth status determination: " + opts.client_type);
  }
