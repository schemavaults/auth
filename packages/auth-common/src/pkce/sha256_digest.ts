...
export async function sha256_digest(code_verifier: string): Promise<string> {
  let digest: ((code_verifier: string) => Promise<string>) | undefined =
    undefined;

  // Load sha-256 digest function
  const isSecureContext: boolean = !!crypto && !!crypto.subtle;
  if (isSecureContext) {
    digest = async function secureContextSha256Digest(
      ...
      const base64url_encoded_hash = base64_encoded_hash.replace(
        /[^A-Za-z0-9_-]/g,
        "_",
      );
    ...
    digest = async function insecureContextSha256Digest(
      code_verifier: string,
    ): Promise<string> {
      let sha256: (code_verifier: string) => Promise<string>;
      try {
        const sha256Digest = await import("crypto-js/sha256").then(
          (mod) => mod.default,
        );
        const base64Stringify = await import("crypto-js/enc-base64").then(
          (mod) => mod.default,
        );
