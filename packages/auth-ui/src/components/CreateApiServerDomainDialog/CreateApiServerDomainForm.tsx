...
/**
 * Rendered inside the dialog's `DialogContent`, so it mounts fresh on every
 * dialog open (Radix unmounts closed dialog content). The form therefore
 * initializes with complete default values — the target API server id from
 * context and a newly generated domain ref id — and never needs a post-open
 * `setValue`, which would re-render the form while the user is typing.
 */
    ...
    <Form {...form}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Register a new domain that this backend API server may be reached
            at.
          </DialogDescription>
        </DialogHeader>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                What is the base URL that this API server may be reached at?
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row gap-2 items-center flex-wrap">
              ...
              <FormDescription className="w-full">
                What environments is this API server domain accessible from?
                This allows restricting production environments to only use a
...
export default CreateApiServerDomainForm;
